...
In many software systems, various actions can only be performed as [some kind of Identity](/centralized-vs-decentralized-identity-tradeoffs.html). We must *authenticate* ourselves by proving *who we are*. **Authentication fundamentally is just an answer to this question: *who are you and can you prove it is true?***
 ...
 * **Auditing & Analytics** - each action may be linked to a particular identity; they might then be tracked and analyzed to increase transparency and understanding of the system: What users (identities) most often do and use? Is their complaint valid? What is worth investing our time? Who has modified this resource at a particular date and time and how? And so on, many more
 * **Identity Lifecycle & Verification** - since there is an identity, more elaborate schemes, statuses and state transitions can be implemented

That is what authentication is on a high level; a fundamental concept of proving one's identity without which many systems and functionalities simply could not be built. **There are multitude possible implementations but fundamentally they are very similar and lead to the same place.** Let's now explore them and see - why it is the case and how it works exactly.
...
* **this session or token now serves as the identity proof** - there is no need to repeat the authentication process on each action/request
...
These are various techniques that generate random and time-limited codes of different lengths - usually numeric or alfanumeric. [They are often used as a second factor in Multi-Factor Authentication setups](#factors-single-vs-multi), but not only - another popular application are *Magic Links*. How does it work there?
...
2. server checks whether associated identity exists; if it does:
    1. short-lived (a few minutes) code is generated
    2. it is saved in the server's database
    3. magic link is constructed - it contains the code & email/phone number/username and redirects to a sign in page; something like: `https://the-best-app.com/sign-in?email=user@email.com&authCode=190b453cx` 
    4. it is sent to the client (us)
3. we receive the magic link
4. we click on it and are redirected to the sign in page
5. on the sign in page, `email` & `authCode` are extracted from query params and sent to the server 
6. [if valid, server returns a Temporary Identity Proof: session or token](#temporary-identity-proofs-sessions-and-tokens)
7. we are authenticated and prove it on each action/request by presenting the received session or token

With magic links, there is no need to store and use passwords, which is very convenient - for both servers and users. **The solution as a whole is as secure as the access to the messaging platform (email, phone number, etc.) is - if somebody hacks it, they are able to take control over our account (identity).** Fortunately, generated codes are short-lived; if they get stolen, most likely they will already expire and no longer be valid for an attack to take place.
...
In the authentication processes described so far - with passwords and codes (magic links) - there was only a single factor (proof). We were supplying a password or temporary code sent to the messaging platform controlled by us: *are there any issues with this approach?*

Well, there is only one factor, a single proof of an identity. If this factor gets stolen, a thief can impersonate us and take control over the identity. 
...
* **[TOTP (Time-based One-Time Password)](https://en.wikipedia.org/wiki/Time-based_one-time_password) authenticator apps** - short-lived codes generated offline, based on the current time and a shared secret (between client and server) - mostly supported and installed on mobile devices. Examples: Google Authenticator, Microsoft Authenticator or Aegis Authenticator
...
* **Trusted contacts & devices** - we might choose a set of trusted contacts who will receive recovery codes in case of problems with our own access; additionally, often *trust this device* feature is supported, allowing us to skip second factor verification on the devices we regularly use
...
This is a slightly different approach, used mainly for machine-to-machine (M2M) authentication. Most commonly, it can be found in:
* SSH protocol
* Mutual TLS (mTLS)
...
Another approach mainly for machine-to-machine (M2M) authentication (and authorization, outside the scope of this post) - mostly used by various REST APIs to validate authenticity of the clients and their access rights. *Sometimes they are called API Tokens, sometimes API Keys*; it is the same thing.
...
* user tries to sign in to service A - they are delegated to service B
...
Now, service A can either use a token or session issued by service B or generate their own, based on the data received from service B. In both cases, **after a successful and fully delegated authentication process, *Temporary Identity Proofs* as sessions or tokens are used**. [From this point on, the flow is the same as described above.](#temporary-identity-proofs-sessions-and-tokens)

## Closing thoughts

We went through pretty much all possible ways of answering the authentication question: *who are you and can you prove it is true?* The question for both humans and machines.

We saw that there are many different *authentication methods* from which we are able to build every imaginable *authentication process* - from simple to complex, requiring single or multiple *authentication factors*.

Excluding [static API Tokens/Keys](#api-tokens-keys), the common pattern arises:
...
* this session, token or ephemeral secret is the *Temporary Identity Proof*, a proof of proof

Which allows to decouple authentication process details from the result - failure or proven identity. Other benefits include improved performance, simplicity, more security and flexibility.

Let’s then close by answering the authentication question - **we (or machines) prove *who we are* by providing various factors: passwords, codes or keys; one, two or sometimes even all of them. We then get a session, token or ephemeral secret that servers as a more flexible and temporary proof of our identity on each action/request.**
...
1. https://www.gomomento.com/blog/api-keys-vs-tokens-whats-the-difference/
3. https://en.wikipedia.org/wiki/Basic_access_authentication
4. https://en.wikipedia.org/wiki/Cryptographic_hash_function#Attacks_on_hashed_passwords
2. https://en.wikipedia.org/wiki/Indirection
3. https://chatgpt.com/c/692ecedd-c8c8-8333-ace0-81e460137902
4. TODO: statefulness of refresh tokens
5. https://openid.net/
6. https://en.wikipedia.org/wiki/JSON_Web_Token
7. https://en.wikipedia.org/wiki/Session_(computer_science)
8. https://auth0.com/blog/demystifying-jose-jwt-family/
9. https://en.wikipedia.org/wiki/Client_certificate
10. https://en.wikipedia.org/wiki/Lightweight_Directory_Access_Protocol
11. https://www.jwt.io/
12. https://en.wikipedia.org/wiki/HMAC
13. https://en.wikipedia.org/wiki/Message_authentication_code
14. TODO: action/request wording
15. authentication factors?
16. HTTP-only cookie WHY note: https://en.wikipedia.org/wiki/HTTP_cookie#Http-only_cookie
17. https://en.wikipedia.org/wiki/One-time_password
18. https://en.wikipedia.org/wiki/Time-based_one-time_password
19. https://en.wikipedia.org/wiki/Multi-factor_authentication
20. https://en.wikipedia.org/wiki/Authentication#Authentication_factors
21. Second factor - more multiple factors mention?
22. https://en.wikipedia.org/wiki/Public_key_infrastructure
23. https://en.wikipedia.org/wiki/Mutual_authentication
24. https://www.cloudflare.com/learning/access-management/what-is-mutual-tls/
25. https://developer.mozilla.org/en-US/docs/Web/HTTP/Reference/Headers/Authorization
26. https://en.wikipedia.org/wiki/Biometrics
27. Kerberos?
28. https://en.wikipedia.org/wiki/API_key
29. More on public key crypto!
