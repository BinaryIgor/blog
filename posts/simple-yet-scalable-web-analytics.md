...
* we need to send rather sensitive data to a third party - IP addresses, users' actions and behaviors; custom solution is far better for privacy
...
```
Explanation:
* `timestamp` - time when an event has occurred
* `ip` - IP address of a client
* `device_id` - unique id of client's device; in our implementation, it is just a random *UUID* stored in the browser *Local Storage*
* `user_id` - id of currently signed-in user or null for anonymous sessions/requests
* `url` - url of the current web page; can help with debugging or checking out path and/or query params
* `browser` - enum: *Chrome, Firefox, Safari, Edge, Opera, etc.*
* `os` - enum describing operating system: *Linux, Windows, Mac, Android, iOS, etc.*
* `device` - enum describing device type: *Mobile, Tablet, Desktop*
* `type` - enum describing event type; it can be anything, for example: *landing-page-view, landing-page-scrolled, sign-up-click, home-page-view, user-profile-view* and so on
* `data` - arbitrary, optional piece of data that can have any schema; it represents additional details that might be useful for some events, such as:
    * `{ "user-profile-id": "484099e3-e5bc-4e66-88d0-98be6e86a53b" }`
    * `{ "view-time": 1000 }`
    * `{ "search-input": "Some Gold" }`

\
There is also an [endpoint that accepts most of the table data](https://github.com/BinaryIgor/code-examples/blob/103f31f00b79b9c727fa472dcf80d3896785cdf2/simple-web-analytics/src/main/java/com/binaryigor/simplewebanalytics/web/WebAnalyticsController.java#L33):
```
...
* `userId` is taken from the *current user session/authToken*; for obvious (security) reasons, only the backend can set it
* `ip` is either extracted from the request by a reverse proxy and set in the `real-ip` header or the backend itself extracts it
* `timestamp` is set by the backend, when an event appears

Also, **we perform [some rudimentary validation](https://github.com/BinaryIgor/code-examples/blob/master/simple-web-analytics/src/main/java/com/binaryigor/simplewebanalytics/core/AnalyticsEventHandler.java#L78), which is crucial, especially considering the fact that this endpoint must be public**. Checks performed here are very basic: whether required fields are null or empty,  or too long - we do not want to be attacked with random events of arbitrary size. We should probably also check whether there is no arbitrary JavaScript in these fields to avoid an [XSS attack](https://owasp.org/www-community/attacks/xss/), if we visualize this data in a UI susceptible to this attack.
...
```
Additionally, the `deviceId` is generated as *UUID* and stored in the *Local Storage*; `device` enum is computed based on the screen size:
```
...
\
It is also trivial to use Grafana or any other visualization tool that supports SQL to create nice-looking dashboards and reports from this data and analyze it there. Nevertheless, regularly running a few raw SQL queries might be more than enough.
...
**[we can easily send more than 1000 event requests per second and save 100 000 events in the following time](https://github.com/BinaryIgor/code-examples/blob/master/simple-web-analytics/generate_random_analytics_events.bash)**:
```
...

2024-08-15T08:04:20.566Z  INFO 1 --- 93000/100000 events were inserted...
2024-08-15T08:04:20.612Z  INFO 1 --- 94000/100000 events were inserted...
2024-08-15T08:04:20.700Z  INFO 1 --- 95000/100000 events were inserted...
2024-08-15T08:04:20.851Z  INFO 1 --- 96000/100000 events were inserted...
2024-08-15T08:04:20.921Z  INFO 1 --- 97000/100000 events were inserted...
2024-08-15T08:04:20.994Z  INFO 1 --- 98000/100000 events were inserted...
2024-08-15T08:04:21.147Z  INFO 1 --- 99000/100000 events were inserted...
2024-08-15T08:04:21.183Z  INFO 1 --- 100000/100000 events were inserted...
2024-08-15T08:04:21.183Z  INFO 1 --- 100000 events inserted! It took: PT13.277879125S
...
It is also happening  with rather limited db (1 CPU, 100% available) and backend (4 CPUs, 400% available) resources consumption:
```
...
As we saw, creating a simple yet scalable web analytics solution from scratch is not that complicated. But is it *production-ready*? Yes, there are just a few things to be mindful of:
* `POST: /analytics/events` endpoint must be public - we need to take care of the input validation, mainly limiting its size as we did here but **we should also add rate limiting**, which can be easily done with the help of a reverse proxy (Nginx for example). Without it, we are susceptible to a [Denial of Service attack](https://en.wikipedia.org/wiki/Denial-of-service_attack): for example, somebody might send us lots of random event requests exhausting our database's disk space and we will no longer be able to save any events
* Parsing `userAgent` string on the frontend can get quite tricky; as an alternative, we can parse the `User-Agent` header on the backend, which is sent automatically by the browser
* If we expect to have significantly higher load for analytics events compared to the regular traffic on our app's backend - it might be a wise idea to scale horizontally and deploy analytics backend separately and not as a part of our app's backend, since it can slow it down. Scaling vertically, by just giving our backend more resources to handle additional analytics traffic is another valid option
* **Have separate analytics and application databases** - higher load on the analytics database should not have an impact on our app
* Presented here Java + Postgres implementation is just an example. This approach is generic and can be reimplemented in any backend programming language with any JSON-supporting SQL database
* In the event of a longer, not temporary, database crash, we might lose some events - if we insist on having 100% guarantee that all analytics events are saved, more resilient code should be added. It would save events temporarily to a file or another, alternative storage and move them to the database once it is healthy again; but let's keep in mind that these are only analytical events: having a redundant setup like this is most likely not worth its complexity

**Keeping these points in mind, we can enjoy the power of JSON in SQL and a batch insert trick to handle thousands of events per second with this straightforward yet highly scalable, private, flexible, and generic approach.**
...
2. JSON support in various SQL databases:
    ...
    4. SQLite: https://www.sqlite.org/json1.html
    5. SQL Server: https://learn.microsoft.com/en-us/sql/relational-databases/json/json-data-sql-server
    6. Oracle: https://docs.oracle.com/en/database/oracle/oracle-database/23/adjsn/json-in-oracle-database.html
3. User Agent:
