...
SQLite is an embedded database that lives as a library, inside the process and runs together with it; it writes and reads from a single database file.
...
But, as we are about to explore, SQLite might just be a better option for the vast majority of the systems/applications out there. It does have limitations - we can access it from a single machine for example, since it is just a local file.
To examine whether SQLite can be treated as a drop-in replacement for relational database management systems (RDBMS), we will go through:
...
As we can see, writes are limitations here  - all writes in SQLite are serialized, there is only one writer at any given time. 
...
Virtual Private Server (VPS) is not really a single physical machine - it is a single logical machine, with many levels of redundancy, both hardware and software wise, implemented by cloud providers to gurantee high availability. Most cloud providers, guarantee *at least 99.9% availability* in their service-level agreements (SLA) and some - [DigitalOcean](https://docs.digitalocean.com/products/droplets/details/sla/) and [AWS](https://aws.amazon.com/compute/sla/) for example - offer *99.99% availability*. That comes down to:
```
...
**Depending on the chosen cloud provider, this is the availability we can expect for a simplest possible system, running on a single virtual server.** What if that is not enough for us? Or maybe we simply do no trust these claims and want to have more redundancy but still enjoy the benefits of SQLite simplicity?

There are multiple ways of increasing availability and redundancy of a single machine, SQLite based system. First, the easiest one for short unavailability - up to a few seconds. This can be fixed purely on the client side by having retries - for the user certain requests will be just slower - because of possible, short server unavailability - but they will suceeded eventually. What about the longer ones?
...
* Availability 99.99 - with proper retries strategy, we can most likely increase it to 99.999 (DO droplets and volumes)
